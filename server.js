...
- personalityはキャラクターの見た目や雰囲気から推定すること（cool=クール・無表情・知的, cute=可愛い・幼い, energetic=元気・活発, calm=穏やか・おっとり, serious=真面目・厳格）`;
      ...
      for (const p of metadata.panels) {
        if (p.dialogues) {
          for (const d of p.dialogues) {
            tempDialoguesForLang.push(d);
          }
        }
      }
    }
    const detectedLang = detectLanguage(tempDialoguesForLang);
    ...
    if (metadata && metadata.panels && metadata.panels.length > 0 && apiKey) {
      try {
        if (isEnglish) {
          sessionLog(sessionId, `📝 [OCR Correction] 2-Pass目: AIによる英語コンテキスト校正を開始中...`);
        } else {
        ...
        const correctionPrompt = isEnglish
          ? `You are an excellent manga editor and proofreader.
Analyze the Vision OCR transcription results of the manga and correct any spelling mistakes, typos, or character recognition errors.

【Context】
- Since the image is generated by AI, text inside speech bubbles or titles may have recognition errors (e.g., mismatched characters or typos).
- Please correct spelling/typos to make it natural English based on the flow of the story.

【Rules】
1. Maintain the character's tone, dialect, or personality. Do NOT summarize or change the meaning.
2. Only fix typos/spelling errors. If the text is already correct English, do NOT modify it.
3. Keep all names, speakers, and the title in English. Do NOT translate anything into Japanese or other languages.
4. Set the 'pronunciation' field to be EXACTLY the same as the corrected 'text' field (since we use Edge-TTS for English).
5. Output ONLY the corrected JSON structure containing 'title' and 'panels'. Do not include markdown code blocks.

【JSON to Correct】
${JSON.stringify(correctionInput, null, 2)}`
          : `あなたは非常に優秀なマンガ編集者および校正・校閲AIです。
入力された漫画のVision OCRの文字起こし結果を読み、日本語として自然で前後のストーリーの文脈に合うように、誤字脱字や文字の誤認識（文字の滲みや歪みによる誤読）を校正してください。
...
${JSON.stringify(correctionInput, null, 2)}`;
    ...
    if (isEnglish) {
      ...
      if (titleJpChars > 0 && metadata.title.length > 0) {
        sessionLog(sessionId, `🔧 [OCR Corrected Title] タイトルが日本語に翻訳されているのを検知しました: "${metadata.title}"`);
        
        let englishTitleCandidate = "";
        const rawTexts = tempDialoguesForLang.map(d => d.text || '');
        for (const t of rawTexts) {
          if (/^[A-Za-z\s\-\:\'\!\.\,\?]{10,}$/.test(t.trim()) && !t.toUpperCase().includes('I WANTED') && !t.toUpperCase().includes('I GUESS')) {
            englishTitleCandidate = t.trim();
            break;
          }
        }
        if (!englishTitleCandidate) {
          englishTitleCandidate = "AI GENERATED MANGA";
        }
    ...
    metadata = {
      ...
      panels: (metadata.panels || []).map((p, i) => ({
        ...
        dialogues: (p.dialogues || []).map(d => {
          ...
          if (isEnglish) {
            // ナレーションは英語に変更
            const localNarratorPatterns = ['ナレ', 'narr', '語り手', '地の文', 'ナレーター', 'ナレータ'];
            if (localNarratorPatterns.some(pat => speaker.toLowerCase().includes(pat.toLowerCase()))) {
              speaker = 'Narrator';
            } else {
              // 主要な日本語話者名の英語訳マッピング
              const translations = {
                '黒髪の女性': 'Black-haired woman',
                '金髪メガネの女性': 'Blonde glasses woman',
                'オレンジ髪の女性': 'Orange-haired woman',
                '茶髪メガネの女性': 'Brown glasses woman',
                '金髪ギャルの女性': 'Blonde gal woman',
                '金髪の女性': 'Blonde woman',
                '赤髪の女性': 'Red-haired woman',
                '眼鏡の女性': 'Glasses woman',
                '不明': 'Unknown',
                '張り紙': 'Paper sign',
                '看板': 'Signboard',
              ...
              if (translations[speaker]) {
                speaker = translations[speaker];
              } else {
                // 部分一致や一般的な置換
                let translated = speaker;
                const subTranslations = [
                  [/黒髪の女性/g, 'Black-haired woman'],
                  [/金髪の女性/g, 'Blonde woman'],
                  [/赤髪の女性/g, 'Red-haired woman'],
                  [/眼鏡の女性/g, 'Glasses woman'],
                  [/金髪メガネの女性/g, 'Blonde glasses woman'],
                  [/オレンジ髪の女性/g, 'Orange-haired woman'],
                  [/茶髪メガネの女性/g, 'Brown glasses woman'],
                  [/金髪ギャルの女性/g, 'Blonde gal woman'],
                  [/黒髪/g, 'Black hair '],
                  [/金髪/g, 'Blonde '],
                  [/赤髪/g, 'Red hair '],
                  [/茶髪/g, 'Brown hair '],
                ...
                for (const [reg, rep] of subTranslations) {
                  translated = translated.replace(reg, rep);
                }
                translated = translated.replace(/\s+/g, ' ').trim();
                // まだ日本語が含まれていればフォールバック
                if (/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/.test(translated)) {
                  let fallbackName = 'Character';
                  if (gender === 'female') {
                    fallbackName = age === 'child' ? 'Girl' : 'Woman';
                  } else if (gender === 'male') {
                    fallbackName = age === 'child' ? 'Boy' : 'Man';
                  }
                  translated = fallbackName;
                }
                speaker = translated;
              }
            }
          ...
          if (characterVoiceMap.has(speaker)) {
            ...
            if (isEnglish) {
              // 英語判定された場合は Edge-TTS 音声をアサイン (文字列)
              voiceId = assignEdgeTtsVoice(speaker, gender, usedVoiceIds);
              characterVoiceMap.set(speaker, voiceId);
              sessionLog(sessionId, `🎭 [Casting v2] 検出話者: ${speaker} (${gender}, ${age}, ${personality})`);
              sessionLog(sessionId, `   ↳ 英語判定のため Edge-TTS 音声を選択`);
              sessionLog(sessionId, `   ↳ 重複回避アルゴリズム適用 -> Edge-TTS: ${voiceId} をアサイン`);
            } else {
              if (isNarrator) {
