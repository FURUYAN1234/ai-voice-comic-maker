...
// 長いキーを先にマッチさせるためソート済みの正規表現パターンを構築
...
/**
 * VOICEVOX送信前にテキスト内の英字略語・IT用語をカタカナ読みに変換
 * 大文字小文字を問わずマッチし、辞書の読みに置換する
 */
function applyPronunciationDict(text) {
  return text.replace(PRONUNCIATION_REGEX, (match) => {
    // 大文字小文字を無視して辞書を検索
  ...
  try {
    ...
    for (let i = 0; i < dialogues.length; i++) {
      ...
      try {
        ...
        const voiceText = applyPronunciationDict(d.text);
    ...
    try {
      const titleVoiceText = applyPronunciationDict(title);
