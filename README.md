...
> **This application does NOT work as a hosted web demo.** It requires a local Node.js backend server plus at least one local Japanese TTS engine: VOICEVOX on `localhost:50021` or Supertonic 3 on `localhost:7789`. Unlike browser-only apps (e.g., Nano Banana 2 and ChatGPT Images 2.0 Powered Super AI 4-koma System), this system performs server-side video rendering via Remotion (headless Chromium) and communicates with local TTS services, which are impossible from a static hosting service like GitHub Pages.

> **本アプリはWebデモとしては動作しません。** ローカルのNode.jsバックエンドサーバーに加えて、日本語TTSとして `localhost:50021` のVOICEVOX、または `localhost:7789` のSupertonic 3 のどちらかが必要です。他のブラウザ完結型アプリ（Nano Banana 2 and ChatGPT Images 2.0 Powered Super AI 4-koma System等）とは異なり、本システムはRemotionによるサーバーサイド動画レンダリング（ヘッドレスChromium）とローカルTTSサービスとの通信を行うため、GitHub Pagesのような静的ホスティングでは動作できません。
