...
v1.1.4 — AI-driven 4-koma manga voiceover and video generation tool using Gemini API / Gemini API を使用したAI駆動の4コマ漫画フルボイス動画自動生成ツール
...
Run VOICEVOX / VOICEVOX起動: Ensure VOICEVOX is running locally on port 50021. / VOICEVOXをローカル（ポート50021）で起動しておきます。
Start / 開始: Run `npm install` followed by `npm run dev` to launch the server and browser. / `npm install` の後、`npm run dev` を実行するとサーバーとブラウザが立ち上がります。
