...
### 4. ドキュメント修正時のバージョン管理ルール
- README.mdの文章修正など、**ソースコードに変更を伴わないドキュメントのみの修正**を指示された場合は、バージョンアップ（vX.Y.Zの更新）やフルデプロイ、Release作成、ZIPバックアップ等は行わないこと。
- `git commit` と `git push` のみでリモートに反映させること。
